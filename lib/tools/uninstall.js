...
import { DEFAULT_TOOL_TIMEOUT_MS, MAX_PACKAGES, MAX_SPEC_LENGTH, MAX_TOOL_TIMEOUT_MS } from '../constants.js'
import { ERROR_BRANCH, FOREGROUND_BRANCH, renderAbortedReason, renderForegroundText, renderStopReason } from '../render.js'
    ...
    defineTool({
      ...
      async execute(args, exec) {
        try {
          ...
          const budgetMs = Math.min(Number(args.timeoutMs) > 0 ? Number(args.timeoutMs) : DEFAULT_TOOL_TIMEOUT_MS, MAX_TOOL_TIMEOUT_MS)
          ...
          const venvDir = await resolveExistingVenv(ws, args.venv, extraRoots)
          if (venvDir === null) {
          ...
          try {
            ...
            if (deadline.fired()) {
              return { error: renderStopReason({ tool: 'pyenv_uninstall', operation: 'ensuring pip in ' + venvDir, budgetMs }) }
            }
            const operation = 'pip uninstall ' + specs.join(' ')
            const result = await runForeground(
              subprocess,
              { argv: buildUninstallArgv(venvPython, specs), cwd: ws, stdio: stdioSpec(), graceMs: 5_000, env },
              exec && exec.signal,
            ...
            if (result.exitCode === null) {
              return {
                error: renderAbortedReason({ tool: 'pyenv_uninstall', operation, stdout: result.stdout.text, stderr: result.stderr.text }),
              }
            }
            return {
              kind: 'foreground',
