...
import { pickVenvInterpreter, resolveVenvArg } from '../venv.js'
      ...
      async execute(args, exec) {
        try {
          ...
          if (args.venv !== undefined && String(args.venv).trim().length > 0) {
            venvDir = resolveVenvArg(args.venv, ws)
            let exists = false
            try {
              exists = (await stat(venvDir)).isDirectory()
            } catch {
              // absent — reject below
            }
            if (!exists) throw new Error('venv not found: ' + venvDir)
            if (!(await isVenvDir(venvDir))) throw new Error('not a virtual environment (no pyvenv.cfg): ' + venvDir)
          } else {
