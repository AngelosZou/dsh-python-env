...
export function registerRemoveTool(ctx) {
  ctx.tools.register(
    defineTool({
      ...
      // Keeps the framework budget: deletion is a single fs op with no
      // subprocess to terminate, so the plugin-owned deadline does not apply;
      // the declared 2-minute budget is the backstop.
