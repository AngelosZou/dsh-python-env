...
/** Full `pip install` argv; index/proxy are omitted when not configured. */
export function buildInstallArgv({ venvPython, specs, requirementsPath, indexUrl, proxyUrl }) {
  const argv = [
    venvPython,
    '-m',
    'pip',
 ...
 */
export async function ensurePip(subprocess, venvPython, env, signal) {
  ...
  const boot = await runForeground(
    subprocess,
    { argv: [venvPython, '-m', 'ensurepip', '--upgrade'], stdio: stdioSpec(), graceMs: 60_000, env },
    signal,
  )
...
export async function runInstallAttempts({
  ...
  for (let i = 0; i < attempts.length; i += 1) {
    ...
    const argv = buildInstallArgv({ venvPython, specs, requirementsPath, indexUrl: attempt.indexUrl, proxyUrl })
    const spec = { argv, cwd, stdio: stdioSpec(), graceMs: 15_000, env }
