...
import { guardWorkspacePath } from './guard.js'
 ...
 * path) against the workspace and enforce containment. An empty argument
 * resolves to the workspace root itself and is the caller's responsibility
 * to reject.
 */
export function resolveVenvArg(requested, ws) {
  const text = String(requested ?? '').trim()
  const target = isAbsolute(text) ? text : join(ws, text)
  return guardWorkspacePath(target, ws, 'venv')
}
 ...
 * exist inside the workspace and carry the pyvenv.cfg marker. Without one,
 * discovered environments decide: exactly one wins; several prefer `.venv`
 * and otherwise raise; none returns `null` and the caller decides whether
 * auto-creation is appropriate.
 */
export async function resolveExistingVenv(ws, requested) {
  if (requested !== undefined && String(requested).trim().length > 0) {
    const dir = resolveVenvArg(requested, ws)
    let exists = false
    try {
  ...
  const candidates = (await discoverVenvCandidates(ws)).filter((candidate) => candidate.byMarker)
