 ...
 * `opts.onSpawn` (optional) receives each live probe handle so a caller's
 * deadline watchdog can terminate it.
 */
export async function probeVenvInfo(subprocess, venvDir, env, signal, { withVersion = true, onSpawn } = {}) {
  ...
  if (withVersion && python !== null && valid) {
    try {
      const parsed = await probeVersion(subprocess, [python], signal, onSpawn)
