...
export async function discoverVenvCandidates(rootDir, platform = process.platform) {
  ...
  while (queue.length > 0 && budget > 0) {
    ...
    // The scan root's own name is never a hint: a workspace directory named
    // "venv" or "env" is just a directory. Only the marker counts at depth 0.
    const knownName = depth > 0 && hasKnownName(name, platform)
