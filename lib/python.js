...
export async function probeVersion(subprocess, argv, signal) {
 ...
 */
export async function resolveBaseInterpreter(subprocess, requested, platform = process.platform, signal = undefined) {
  const failures = []
  ...
  for (const candidate of candidates) {
    let version
    try {
      version = await probeVersion(subprocess, candidate.argv, signal)
    } catch (error) {
